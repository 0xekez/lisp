...
impl Expr {
    ...
    pub(crate) fn at_loc(loc: Location, val: ExprVal) -> Self {
        Self { loc, val }
    }
}
...
impl ParseResult {
    ...
    pub(crate) fn from_err(error: Error) -> Self {
        Self {
            expr: None,
            errors: vec![error],
        }
    }
    pub(crate) fn merge_err(&mut self, err: Error) {
...
mod tests {
    ...
    fn single_number() {
        ...
        let res = parser.parse_expr();
        if let Some(e) = res.expr {
            assert_eq!(ExprVal::Number(1), e.val);
        } else {
            assert!(false);
        }
    }
    ...
    fn small_list() {
        ...
        if let Some(e) = res.expr {
            match e.val {
                ExprVal::List(v) => {
                    assert_eq!(v.len(), 3);
                    assert_eq!(v[0].val, ExprVal::Number(1));
