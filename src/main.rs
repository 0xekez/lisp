...
fn parse<'a>(tokens: &'a[String]) -> Result<(LustExpr, &'a[String]), LustErr>
{
    let (token, rest) = tokens.split_first()
        .ok_or(
           LustErr::Reason("Couldn't get token.".to_string())
        )?;
    ...
    data.insert(
        ...
        LustExpr::Func(
            |args: &[LustExpr]| -> Result<LustExpr, LustErr>
            {
                ...
                {
                    return Err(
                        LustErr::Reason("nil? expects only 1 argument.".to_string()))
                }
                let ex = args.first().ok_or(
                    LustErr::Reason("nil? got no arguments.".to_string()))?;
                match ex
                {
                    LustExpr::List(l) => if l.len() == 0
                    {
    ...
    data.insert(
        ...
        LustExpr::Func(
            |args: &[LustExpr]| -> Result<LustExpr, LustErr>
            {
                ...
                {
                    return Err(
                        LustErr::Reason("first expects only 1 argument.".to_string()))
                }
                let ex = args.first().ok_or(
                    LustErr::Reason("first got no arguments.".to_string()))?;
                match ex
                {
                    LustExpr::List(l) => match l.first()
                    {
                        Some(ex) => Ok(ex.clone()),
                        None => Err(
                            LustErr::Reason("first called on empty list.".to_string())),
                    },
                    _ => Err(
                        LustErr::Reason("first called on non-list type.".to_string()))
                }
            }
        )
    );
    data.insert(
        ...
        LustExpr::Func(
            |args: &[LustExpr]| -> Result<LustExpr, LustErr>
            {
                ...
                {
                    return Err(
                        LustErr::Reason("rest expects only 1 argument.".to_string()))
                }
                let ex = args.first().ok_or(
                    LustErr::Reason("rest got no arguments.".to_string()))?;
                match ex
                {
                    LustExpr::List(l) => {
                        match l.len()
                        {
                            0 => Err(
                                LustErr::Reason("rest called on empty list".to_string())),
...
fn eval(expr: &LustExpr, env: &mut LustEnv) -> Result<LustExpr, LustErr>
{
    match expr
    {
        ...
        LustExpr::List(list) =>
        {
            ...
            .ok_or(
                LustErr::Reason("Expected a non-empty list".to_string())
            )?;
            ...
            match eval_builtin_form(first_form, arg_forms, env)
            {
                ...
                None =>
                {
                    ...
                    match first_eval
                    {
                        ...
                        _ => Err(
                            LustErr::Reason("First expresson in list must be a function".to_string())
                        ),
                    }
                }
            } // eval_builtin_form
...
fn env_for_lambda<'a>(params: Rc<LustExpr>, arg_forms: &[LustExpr], outer_env: &'a mut LustEnv)
                      -> Result<LustEnv<'a>, LustErr>
{
    ...
    {
        return Err(
            LustErr::Reason("Wrong number of arguments to lambda expression.".to_string())
        );
    }
...
fn parse_list_of_symbol_strings(form: Rc<LustExpr>) -> Result<Vec<String>, LustErr>
{
    let list = match form.as_ref()
    {
        ...
        _ => Err(
            LustErr::Reason("Expected arguments to be a list.".to_string())
        ),
    }?;
        ...
        .map(
            |x|
            {
                match x
                {
                    ...
                    _ => Err(
                        LustErr::Reason("Only symbols are allowed in argument list".to_string())
                    ),
                }
            }
        ).collect()
...
fn eval_builtin_form(
    exp: &LustExpr, arg_forms: &[LustExpr], env: &mut LustEnv
...
fn eval_eval_args(arg_forms: &[LustExpr], env: &mut LustEnv) -> Result<LustExpr, LustErr>
{
    ...
    let ex = arg_forms.first().ok_or(
        LustErr::Reason("Expected expression in eval.".to_string()))?;
...
fn eval_lambda_args(arg_forms: &[LustExpr]) -> Result<LustExpr, LustErr>
{
    ...
    {
        return Err(
            LustErr::Reason("fn expression can only have two arguments.".to_string())
        )
    }

    let params_expr = arg_forms.first().ok_or(
        LustErr::Reason("Expected lambda arguments.".to_string())
    )?;
    let body_expr = arg_forms.get(1).ok_or(
        LustErr::Reason("Expected lambda body".to_string())
    )?;
...
fn eval_def_args(arg_forms: &[LustExpr], env: &mut LustEnv) -> Result<LustExpr, LustErr>
{
    ...
    {
        return Err(
            LustErr::Reason("def expression can only have two arguments.".to_string())
        )
    }
    
    let first_form = arg_forms.first()
        .ok_or(
            LustErr::Reason("Expected first form in def".to_string()),
        )?;
    ...
    let first_str = match first_form
    {
        LustExpr::Symbol(s) => Ok(s.clone()),
        _ => Err(
            LustErr::Reason("Expected first form to be a symbol.".to_string()))
    }?;

    let second_form = arg_forms.get(1)
        .ok_or(
            LustErr::Reason("Expected second form in def".to_string())
        )?;
